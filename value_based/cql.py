...
class CQLAgent():
    def __init__(self,
        ...
        self.scheduler = ReduceLROnPlateau(self.optimizer, mode='min', factor=factor, patience=patience, threshold=improve)
        ...
        self.curr_Qnet.apply(self._init_weights)
    ...
    def _get_action_train(self, net, states, legal_max_actions):
        Q = net(states)
    ...
    def learn(self, experiences, train_for):
        ...
        curr_argmax_next_a = self._get_action_train(self.curr_Qnet, next_states, legal_next_max_actions)  # argmax_a' Q(s',a';θ)
        with torch.no_grad():
        ...
        curr_Q = self.curr_Qnet(states).gather(1, actions.unsqueeze(1)).squeeze()  # Q(s,a;θ) given a

        # loss function 对应原文eq.(4)
        cql_loss = self.alpha * (torch.logsumexp(self.curr_Qnet(states), dim=1).mean() - self.curr_Qnet(states).mean())
        # bellman_error = 0.5 * F.mse_loss(curr_Q, targ_Q)
        bellman_error = F.mse_loss(curr_Q, targ_Q)
        q_loss = cql_loss + bellman_error

        self.optimizer.zero_grad()
        q_loss.backward()
        clip_grad_norm_(self.curr_Qnet.parameters(), 1)
        self.optimizer.step()
        self.scheduler.step(q_loss)

        # soft update for target network
        self._soft_update(self.curr_Qnet, self.targ_Qnet)

        return q_loss.detach().item(), cql_loss.detach().item(), bellman_error.detach().item()
