...
def get_config():
    ...
    parser.add_argument("--exp_config", type=str, default="configs/ddqn_w_config.txt", help="The path where the config file saves.")
...
def train(args, e_tensordata):
    ...
    if args['use_wandb']:
        ...
        train_metrics = ["Bellman Error "+args['for']]
    ...
    agent = DDQNAgent(**args['DDQNAgent_param'])
    ...
    for epoch in range(args['epoch']):
        ...
        for partition_id in partition_order:
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                bellman_error = agent.learn((states, actions, rewards_w, rewards_r, next_states_w, next_states_r, \
                                             legal_max_actions, legal_next_max_actions_w, legal_next_max_actions_r), \
                                             args['for'])
                bellman_error_list.append(bellman_error)
                if args['use_wandb']:
                    wandb.log({"Bellman Error "+args['for']: bellman_error,
    ...
    with open(intermediate_result_filename, "wb") as f:
        pickle.dump({'bellman_error_'+args['for']: bellman_error_list,
...
if __name__ == "__main__":
    ...
    # nohup python -u train_ddqn.py --exp_config configs/ddqn_w_config.txt> logs/ddqn_w_0701_xxxx_s1111.log 2>&1 &
