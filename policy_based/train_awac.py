...
def get_config():
    ...
    parser.add_argument("--exp_config", type=str, default="configs/awac_w_config.txt", help="The path where the config file saves.")
...
def train(args, e_tensordata):
    ...
    if args['use_wandb']:
        ...
        train_metrics = ["Q Loss "+args['for'], "A Loss "+args['for']]
    ...
    agent = AWACAgent(**args['AWACAgent_param'])

    epoch_cnt, batch_cnt, eval_cnt = 0, 0, 0
    q_loss_list, a_loss_list = [], []
    ...
    for epoch in range(args['epoch']):
        ...
        for partition_id in partition_order:
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                if batch_cnt % args['AWACAgent_param']['a_update_frequency'] == 0:
                    q_loss, a_losses = agent.learn((states, actions, rewards_w, rewards_r, next_states_w, next_states_r, \
                                                    legal_max_actions, legal_next_max_actions_w, legal_next_max_actions_r), \
                                                    args['for'], batch_cnt)
                    q_loss_list.append(q_loss); a_loss_list.extend(a_losses)
                else:
                    q_loss, _ = agent.learn((states, actions, rewards_w, rewards_r, next_states_w, next_states_r, \
                                            legal_max_actions, legal_next_max_actions_w, legal_next_max_actions_r), \
                                            args['for'], batch_cnt)
                    q_loss_list.append(q_loss)
                
                if args['use_wandb']:
                    wandb.log({"Q Loss "+args['for']: q_loss, "A Loss "+args['for']: a_loss_list[-1],
                ...
                if batch_cnt % args['print_frequency'] == 0:
                    print("E[%d/%d]B[%5d/%d]%s -- Q Loss: %.6f | A Loss: %.6f \n\t\t\t Abso CR: %.5f | Rela CR: %.5f Rew: %.3f\n\t\t\t q_lr: %.4e | a_lr: %.4e" \
                          % (epoch_cnt, args['epoch'], batch_cnt, (args['sample_num']//args['train_batch_size']+27)*args['epoch'], args['for'], \
                             q_loss, a_loss_list[-1], abso_CR, rela_CR, total_reward, \
                             agent.q_optimizer.param_groups[0]["lr"], agent.a_optimizer.param_groups[0]["lr"]))
...
if __name__ == "__main__":
    ...
    # nohup python -u train_awac.py --exp_config configs/awac_w_config.txt> logs/awac_w_0708_xxxx_s1111.log 2>&1 &
