...
class AWACAgent():
    def __init__(self,
                    max_action_num, category_num, category_emb_dim, industry_num, industry_emb_dim, inner_output_dim,
                    nhead_1, nhead_2, encoder_layer_num, feature_dict,
                    tau, gamma, a_update_frequency, awac_lambda, exp_adv_max, 
        ...
        self.a_scheduler = ReduceLROnPlateau(self.a_optimizer, mode='min', factor=a_factor, patience=a_patience, threshold=a_improve)
        self.q_scheduler = ReduceLROnPlateau(self.q_optimizer, mode='min', factor=q_factor, patience=q_patience, threshold=q_improve)
    ...
    def _get_action_train(self, actor, states, legal_max_actions):
        ...
        policy_dist = Categorical(logits=logits_masked)
        # actions = policy_dist.sample()
        a_probs = policy_dist.probs
        actions = torch.argmax(a_probs, dim=1)
        a_log_probs = F.log_softmax(logits_masked, dim=1)     
    ...
    def learn(self, experiences, train_for, batch_cnt):
        ...
        # TRAIN CRITIC
        with torch.no_grad():
            next_actions, _, _ = self._get_action_train(self.actor, next_states, legal_next_max_actions)  # a'
            # minQ(s',a';θ') given a'
            min_targ_next_q = torch.min(self.targ_Qnet1(next_states).gather(1, next_actions.unsqueeze(1)).squeeze(),
                                        self.targ_Qnet2(next_states).gather(1, next_actions.unsqueeze(1)).squeeze())  
            # y = r + γ * minQ(s',a';θ')
            min_targ_q = rewards + self.gamma * min_targ_next_q
        ...
        if batch_cnt % self.a_update_frequency == 0:
            for _ in range(self.a_update_frequency):  # Loop time is same as update frequency
                with torch.no_grad():
                    pi_actions, _, _ = self._get_action_train(self.actor, states, legal_max_actions)  # a
                    v = torch.min(self.curr_Qnet1(states).gather(1, pi_actions.unsqueeze(1)).squeeze(),
                                  self.curr_Qnet2(states).gather(1, pi_actions.unsqueeze(1)).squeeze())  # V(s) a是actor采样得到的 近似代表V(s)
                    q = torch.min(self.curr_Qnet1(states).gather(1, actions.unsqueeze(1)).squeeze(),
                                  self.curr_Qnet2(states).gather(1, actions.unsqueeze(1)).squeeze())  # Q(s,a) a是数据集提供的 是关于a确定的Q(s,a)
                    adv = q - v  # A(s,a) = Q(s,a) - V(s)
                    weights = torch.clamp_max(torch.exp(adv / self.awac_lambda), self.exp_adv_max)  # exp(1/λ * A(s,a))
                _, _, actions_log_probs = self._get_action_train(self.actor, states, legal_max_actions)  # logΠ(a|s)
                action_log_probs = actions_log_probs.gather(1, actions.unsqueeze(1)).squeeze()  # logΠ(a|s) given a
                a_loss = (-1 * action_log_probs * weights).mean()  # maximize E_{s,a}[logΠ(a|s) * exp(1/λ * A(s,a))]

                self.a_optimizer.zero_grad()
                a_loss.backward()
                clip_grad_norm_(self.actor.parameters(), 1)
                self.a_optimizer.step()
                # self.a_scheduler.step(a_loss)
